...
class GenericPermissions(tornado.web.RequestHandler):
	...
	def post(self):
		...
		module_id = post_arguments.get(self.document_id_name, None)
		logger.info(post_arguments)
		try:
			assert isinstance(permissions, list), "permissions must be an array of objects"
			assert module_id is not None, "%s cannot be left empty"%self.document_id_name
			...
			yield if_module_permission(self.child_collection, user_id, "create", module_id, self.document_id_name)
			
			logger.info("The user [%s] have create permission on %s [%s]"%(user_id, self.document_id_name, module_id))

			for permission_obj in permissions:
					user_id = permission_obj.pop("user_id")
					update_module_collection = yield self.child_collection.update_one({self.document_id_name: module_id}, \
					...
					update_user_collection = yield self.user_collection.update_one({"user_id": user_id}, \
						{"$set": {"permissions.%s.%s"%(self.document_name, level_id): permission_obj}}, upsert=False)
			...
			self.finish()
			return 
		self.write({"error": False, "success": True, self.document_id_name: module_id, \
						"message": "Permissions updated for %s with id=[]"%(self.document_id_name, module_id)})
		self.finish()
		return 
...
class Generic(tornado.web.RequestHandler):
	...
	def post(self):
			...
			parent_id = post_arguments.get(self.document_id, None)
			...
			try:
					...
					child_document = yield self.child_collection.find_one({self.document_name: module_name})

					if child_document:
						raise Exception("%s on %s module already exists"%(module_name, self.document_name))

					child_hash = "%s%s"%(user_id, module_name)

					child_id = hashlib.sha1(child_hash.encode("utf-8")).hexdigest()
					

					parent["parents"].insert({"name": parent["module_name"], "id": parent["module_id"] })
					logger.info(parent["parents"])
					result = yield self.child_collection.insert_one({"module_id": child_id, \
					...
					logger.info("New [%s] with name=[%s], _id=[%s] and module_id=[%s] created by user id [%s]"%(self.document_name, module_name,\
					 result.inserted_id, child_id, user_id))
					...
					update_child_collection = yield self.child_collection.update_one({"module_id": child_id}, \
											{"$set": {user_id: permission}}, upsert=False)

					update_user_collection = yield self.user_collection.update_one({"user_id": user_id}, \
						{"$set": {"permissions.%s.%s"%(self.document_name, level_id): permission}}, upsert=False)
				...
				self.finish()
				return 
			self.write({"error": False, "success": True, "message": None, self.document_id: child_id, \
	...
	def get(self, module_id):
			...
			try:
				yield if_module_permission(self.child_collection, user_id, "get", module_id, self.document_id)

				result = yield self.child_collection.find_one({self.document_id: module_id}, projection={"_id": False, 
	...
	def put(self, module_id):
			...
			user_id = post_arguments.get("user_id", None) ##who created this category
			try:
				yield if_module_permission(self.child_collection, user_id, "put", module_id, self.document_id)

				self.child_collection.update_one({self.document_id: module_id}, {"$set":{"text_description": text_description,\
				...
				self.finish()
				return 
			self.write({"error": False, "success": True, "message": None, self.document_id: module_id})
			self.finish()
			return 
	...
	def delete(self, module_id):
			...
			user_id = post_arguments.get("user_id", None) ##who created this category
			try:
				yield if_module_permission(self.child_collection, user_id, "delete", module_id, self.document_id)
			except Exception as e:
				print (traceback.format_exc())
				logger.error(e)
				self.write({"error": True, "success": False, "message": e.__str__()})
				self.finish()
				return 
			self.write({"error": False, "success": True, self.document_id: module_id})
			self.finish()
			return 
