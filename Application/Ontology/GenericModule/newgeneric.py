...
class Generic(tornado.web.RequestHandler):
	...
	def  post(self):
		...
		try:
			...
			is_domain = yield Permission.is_domain(self.module_type, user):
			if not is_domain: ##this implies that other type of module is being created
				...
				yield Permission.is_not_domain(user, parent_module, self.permission_collection)
	...
	def get(self, module_id=None):
		...
		if user["user_type"] == "superadmin":
			if text_search:
				modules = yield module_collection.find({"user_id": user["user_id"]},{"module_name":{"$text":{"$search": text_search}}}).skip(skip).limit(limit)
			else:
			...
			[module.update({"permission": {"get": True, "delete": True, "add_child": True, "edit": True}}) for module in modules]
			    ...
			    modules = yield Permission.get_modules(user, skip, limit, textsearch, self.module_collection, self.permission_collection)

		message = {"error": True, "success": False, "message": message, "data": modules}
		self.write(message)
		logger.info(message)
		self.finish()
		return
